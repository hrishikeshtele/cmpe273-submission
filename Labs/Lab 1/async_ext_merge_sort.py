...
class Driver:
    ...
    # Read input files
    async def readFiles(self):
        # list of input file names
        tasks = []
        for input_file in glob.glob("input/unsorted_*.txt"):
            tasks.append(asyncio.create_task(self.readFileTask(input_file)))
        await asyncio.gather(*tasks)

    async def readFileTask(self, input_file):
    ...
    def mergeSortedTempFiles(self):
        ...
        sorted_file = open(glob.glob("output/")[0] + "async_sorted.txt", "w")
        ...
        while True:
            ...
            if cnt == 100:
                with open(glob.glob("output/async_sorted.txt")[0], "a") as f:
